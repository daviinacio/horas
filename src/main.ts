...
config.setDefault(
  "template_folder",
  `${config.get("timesheet_folder")}/Templates`
);
...
const command_create = program
  .command("criar")
  .description("Comando para criação de arquivo de horas")
  .addHelpCommand("ajuda", "Ajuda nos comandos de criação");

command_create.command("proxima").addCommand(
  createCommand("semana")
    .description("Criar controle para todos os dias úteis da próxima semana")
    .action(() => actions.create.week(1))
);

command_create.command("proximo").addCommand(
  createCommand("mes")
    .description("Criar controle para todos os dias úteis do próximo mês")
...
command_create
  ...
  .addCommand(
    createCommand("passada")
      .description(
        "Criar controle para todos os dias úteis da semana passada (semana atual -1)"
      )
      .action(() => actions.create.week(-1))
  )

  .addCommand(
    createCommand("retrasada")
      .description(
        "Criar controle para todos os dias úteis da semana retrasada (semana atual -2)"
      )
...
command_create
  ...
  .addCommand(
    createCommand("passado")
      .description(
        "Criar controle para todos os dias úteis do mês passado (mês atual -1)"
      )
      .action(() => actions.create.month(-1))
  )

  .addCommand(
    createCommand("retrasado")
      .description(
        "Criar controle para todos os dias úteis do mês retrasado (mês atual -2)"
      )
      .action(() => actions.create.month(-2))
  );

const command_calculate = program
  .command("calcular")
